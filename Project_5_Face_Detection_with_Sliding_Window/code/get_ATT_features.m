...
function features_pos = get_ATT_features(train_path_pos, feature_params)
...
% for i=1:40
%     fname= strcat('s',num2str(i));
%     image_files=dir(fullfile(fname,'/*.pgm'));
%     for j=1:size(image_files,1)
%         img =imread( fullfile( image_files(j).folder, image_files(j).name));
%         imwrite(img,strcat('s',num2str(i),'_',num2str(j),'.jpg'));
%     end
% end
