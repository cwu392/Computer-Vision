...
function [bboxes, confidences, image_ids] = .... 
    run_detector_NN(test_scn_path, X, Y, feature_params)
...
for i = 1:length(test_scenes)
    ...
    for y = 1 : floor(size(hog, 1)/(temp_size/cell_size)-1)
        for x = 1 : floor(size(hog, 2)/(temp_size/cell_size)-1)
            hog_feat = hog(y : (y + temp_size/cell_size - 1), x : (x + temp_size/cell_size - 1), :);
            hog_feat = reshape(hog_feat, [1,D]);
            conf = nearest_neighbor_classify(X, Y, hog_feat);
            if (conf > threshold)
                x_min = (x-1)*cell_size;
                y_min = (y-1)*cell_size;
