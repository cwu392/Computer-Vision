...
tic;
%% Setup
test_image = im2single(imread('../data/motorcycle.bmp'));
...
identity_image  = my_imfilter_fast(test_image, identity_filter);

figure(2); imshow(identity_image);
imwrite(identity_image, './Motorcycle/identity_image.jpg', 'quality', 95);
...
blur_image = my_imfilter_fast(test_image,blur_filter);

figure(3); imshow(blur_image);
imwrite(blur_image, './Motorcycle/blur_image.jpg', 'quality', 95);
...
large_blur_image = my_imfilter_fast(test_image, large_1d_blur_filter);
large_blur_image = my_imfilter_fast(large_blur_image, large_1d_blur_filter'); %notice the ' operator which transposes the filter

figure(4); imshow(large_blur_image);
imwrite(large_blur_image, './Motorcycle/large_blur_image.jpg', 'quality', 95);
...
sobel_image = my_imfilter_fast(test_image, sobel_filter);

%0.5 added because the output image is centered around zero otherwise and mostly black
figure(5); imshow(sobel_image + 0.5);
imwrite(sobel_image + 0.5, './Motorcycle/sobel_image.jpg', 'quality', 95);


%% High pass filter (Discrete Laplacian)
laplacian_filter = [0 1 0; 1 -4 1; 0 1 0];

laplacian_image = my_imfilter_fast(test_image, laplacian_filter);

%0.5 added because the output image is centered around zero otherwise and mostly black
figure(6); imshow(laplacian_image + 0.5);
imwrite(laplacian_image + 0.5, './Motorcycle/laplacian_image.jpg', 'quality', 95);

%% High pass "filter" alternative
high_pass_image = test_image - blur_image; %simply subtract the low frequency content

figure(7); imshow(high_pass_image + 0.5);
imwrite(high_pass_image + 0.5, './Motorcycle/high_pass_image.jpg', 'quality', 95);
toc
%by James Hays