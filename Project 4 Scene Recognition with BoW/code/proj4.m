...
% FEATURE = 'gaussian fisher encoding'; 
% FEATURE = 'spyramid fisher encoding'; %Optimum Performance
% FEATURE = 'placeholder';

% CLASSIFIER = 'nearest neighbor';
% CLASSIFIER = 'support vector machine';
 CLASSIFIER = 'svm rbf kernel';
% CLASSIFIER = 'svm rbf kernel libsvm'; %Optimum Performance
...
switch lower(FEATURE)    
    ...
    case 'fisher encoding'
        if ~exist('stats.mat', 'file')
            fprintf('No existing stats found. Computing one from training images\n')
            vocab_size = 256; %From 'http://www.robots.ox.ac.uk/~vgg/research/encoding_eval/'
            stats = build_gmm(train_image_paths, vocab_size);
            save('stats.mat', 'stats')
        end
        
        % YOU CODE get_fisher_encoding.m
        train_image_feats = get_fisher_encoding(train_image_paths);
        test_image_feats  = get_fisher_encoding(test_image_paths);
        
    case 'gaussian fisher encoding' 
        % To reduce function complexity, named gaussian_status as status.
        if ~exist('stats.mat', 'file')
            fprintf('No existing gaussian stats found. Computing one from training images\n')
            vocab_size = 400; %From 'http://www.robots.ox.ac.uk/~vgg/research/encoding_eval/'
            stats = build_gaussian_gmm(train_image_paths, vocab_size);
            save('stats.mat', 'stats')
        end
    ...
    case 'spyramid fisher encoding' 
        % To reduce function complexity, named gaussian_status as status.
        if ~exist('stats.mat', 'file')
            fprintf('No existing gaussian stats found. Computing one from training images\n')
            vocab_size = 400; %From 'http://www.robots.ox.ac.uk/~vgg/research/encoding_eval/'
            stats = build_spyramid_gmm(train_image_paths, vocab_size);
            save('stats.mat', 'stats')
        end
        
        % YOU CODE get_fisher_encoding.m
        train_image_feats = get_spyramid_fisher_encoding(train_image_paths);
        test_image_feats  = get_spyramid_fisher_encoding(test_image_paths);    
...
switch lower(CLASSIFIER)    
    ...
    case 'svm rbf kernel'
        % YOU CODE svm_rbf_kernel.m 
        predicted_categories = svm_rbf_kernel(train_image_feats, train_labels, test_image_feats);
        
    case 'svm rbf kernel libsvm'
        % YOU CODE svm_rbf_kernel.m 
        predicted_categories = svm_rbf_kernel_libsvm(train_image_feats, train_labels, test_image_feats, test_labels);    
