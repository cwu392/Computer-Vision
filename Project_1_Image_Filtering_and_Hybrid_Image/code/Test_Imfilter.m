% this script has test cases to help you test imfilter() which you will
...
% imfilter() if you want to check that imfilter() is doing something
% similar.
clear all
close all
tic;
...
identity_image  = imfilter(test_image, identity_filter);
...
large_blur_image = imfilter(test_image, large_1d_blur_filter);
large_blur_image = imfilter(large_blur_image, large_1d_blur_filter'); %notice the ' operator which transposes the filter
...
% large_blur_image = imfilter(test_image, large_blur_filter);
% toc 
...
laplacian_image = imfilter(test_image, laplacian_filter);
