% this script has test cases to help you test my_imfilter_resize() which you will
...
% imfilter() if you want to check that my_imfilter_resize() is doing something
% similar.
clear all
close all
tic;
...
identity_image  = my_imfilter_resize(test_image, identity_filter);
...
blur_image = my_imfilter_resize(test_image,blur_filter);
...
%large_1d_blur_filter = fspecial('Gaussian', [3 3], 10);

%large_blur_image = my_imfilter_resize(test_image, large_1d_blur_filter);
%large_blur_image = my_imfilter_resize(large_blur_image, large_1d_blur_filter'); %notice the ' operator which transposes the filter
...
% large_blur_image = my_imfilter_resize(test_image, large_blur_filter);
% toc 

%% Oriented filter (Sobel Operator)
sobel_filter = [-1 0 1; -2 0 2; -1 0 1]; %should respond to horizontal gradients

sobel_image = my_imfilter_resize(test_image, sobel_filter);
...
laplacian_image = my_imfilter_resize(test_image, laplacian_filter);
